...
usuarios = {
    "professor1": {"senha": "1234", "tipo": "professor"},
    ...
    try:
        ...
        if tipo_usuario == "professor":
            for _, row in df.iterrows():
                tree.insert("", "end", values=(row["Aluno"], row["Nota1"], row["Nota2"], row["Situação"]))
...
def excluir_aluno(tree):
    if usuarios["professor"]["tipo"] != "prefessor":
        messagebox.showwarning("Acesso Negado", "Somente professore podem excluir registros.")
        return
